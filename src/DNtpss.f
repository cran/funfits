      subroutine DNtpss(des,lddes,nobs,dim,m,s,lds,ncov,y,wt,
     * ntbl,adiag,
     * lamlim,dout,iout,coef,svals,tbl,ldtbl,auxtbl,work,lwa,
     * iwork,liwa,job,jobrd,info)
c                       |
c                 difference between DNtpss and dtpss
c
     ...
     * tbl(ldtbl,3),auxtbl(3,3),work(lwa),wt(nobs)
      integer save(5)
c
...
c 	smoothing spline matrix decompositions will not be recomputed
c       if jobrd=1.
c
c  See comments in tpss.f for documanetation
c On Entry:
...
c
c   jobrd
c         if jobrd=0 then decompostions will be computed
c         if jobrd =1  and will use current set if available   DN 4/2/87
c
c On Exit:
...
c			  2  	V(lambda)
c			  3     traceA(lambda) if requested
...
c
c $Header: /usr/local/cvsroot/funfits/src/DNtpss.f,v 1.1.1.1 1998/05/24 21:50:07 agebhard Exp $
c        changed by D. Nychka 4/2/87 (jobrd) 11/4/87 (wt)
c
...
c			rest of iwork is a integer work vector
      ...
      if (jrep .gt. 0) then
         call DNreps(des,lddes,nobs,dim,s,lds,ncov,0,work,wt,
     *    iwork,nuobs,
     *    iwork(ip1p1),jrep,info)
         if (( nobs-nuobs.gt.0).and.(jrep.eq.2)) info=1
         if (info .ne. 0) then
            info = 2
            return
         endif
...
c
      ...
      npsing = nuobs - ncts
       ...
       if( (save(1).eq.0)
     *       .or.(jobrd.eq.0).or.(save(2).ne.nobs) ) then

c ********** block if decompostions are to be done

      write(*,*)  'call to setup in tpss'
...
c****** set save(1)=1  to indicate decompostions have been done 
       save(1)=1
       save(2)=nobs
...
c
c
c******** point where calculations will always be repeated
c
c			setup y
      if (jrep .eq. 1) then
